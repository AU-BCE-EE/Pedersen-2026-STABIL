...
data <- data[data$date.time >= as.POSIXct("2022-10-25 09:20:00", tz="UTC"), ]
...
data$treat <- mapvalues(data$treat, from = c('4', '9', '18'), to = c(rep('PS-LB1', 3)))
data$treat <- mapvalues(data$treat, from = c('3', '12', '16'), to = c(rep('PS-LB2', 3)))
data$treat <- mapvalues(data$treat, from = c('6', '10', '15'), to = c(rep('PS-L', 3)))
...
data$replicate <- mapvalues(data$replicate, from = c('9', '12', '10' ), to = rep('2', 3))
data$replicate <- mapvalues(data$replicate, from = c('18', '16', '15' ), to = rep('3', 3))
...
#tunnels data 
data <- data[data$treat == 'PS-LB1' | data$treat == 'PS-LB2'| data$treat == 'PS-L', ]
...
#write_xlsx(data[, cols_to_keep_emissions], "../ALFAM2/emission_9.xlsx")
