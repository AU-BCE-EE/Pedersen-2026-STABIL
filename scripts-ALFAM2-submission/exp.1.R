...
# Filter data
data <- data[data$date.time >= as.POSIXct("2022-04-05 12:25:07", tz="UTC"), ]
...
data$treat <- mapvalues(data$treat, from = c('1', '6', '12'), to = c(rep('AD', 3)))
data$treat <- mapvalues(data$treat, from = c('2', '5', '8'), to = c(rep('AD-F', 3)))
data$treat <- mapvalues(data$treat, from = c('4', '9', '10'), to = c(rep('AD-L', 3)))
...
data$replicate <- mapvalues(data$replicate, from = c('6', '5', '9' ), to = rep('2', 3))
data$replicate <- mapvalues(data$replicate, from = c('12', '8', '10' ), to = rep('3', 3))
...
# Remove rows where mpvposition equals 12
...
data<-filter(data, elapsed.time<=142)
...
data$start.date <- data$st
data$end.date <- data$st
data$end.date <- c(data$end.date[-1], data$end.date[nrow(data)]) 
View(data)
# Convert character dates to POSIXct 
data$start.date <- as.POSIXct(data$start.date, format = "%dd-%mm-%YYYY %H:%M", tz = "UTC")
data$end.date <- as.POSIXct(data$end.date, format = "%d-%m-%Y %H:%M", tz = "UTC")
...
#write_xlsx(data[, cols_to_keep_emissions], "../ALFAM2/emission_1.xlsx")
