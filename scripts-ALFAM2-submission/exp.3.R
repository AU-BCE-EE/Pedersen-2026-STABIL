...
# Remove rows where mpvposition equals 12, replicate with loose tube 
data <- data[data$MPVPosition != 12, ]
# Filter data and match Picarro time with Real time
data <- data[data$date.time >= as.POSIXct("2022-09-14 11:08:13", tz="UTC"), ]
data$date.time <- data$date.time + hours(1) + minutes(48)
...
data$treat <- mapvalues(data$treat, from = c('4', '6'), to = c(rep('AD', 2)))
data$treat <- mapvalues(data$treat, from = c('1', '5', '10'), to = c(rep('AD-F', 3)))
data$treat <- mapvalues(data$treat, from = c('2', '8', '9'), to = c(rep('AD-L', 3)))
...
data$replicate <- mapvalues(data$replicate, from = c('4', '1', '2' ), to = rep('1', 3))
data$replicate <- mapvalues(data$replicate, from = c('5', '6', '8' ), to = rep('2', 3))
data$replicate <- mapvalues(data$replicate, from = c('10', '9'), to = rep('3',2))

# background dataset 
data.bg <- data[data$treat == 'bg 1' | data$treat == 'bg 2'| data$treat == 'bg 3', ]

#tunnels data 
data <- data[data$treat == 'AD' | data$treat == 'AD-L'| data$treat == 'AD-F', ]
...
#write_xlsx(data[, cols_to_keep_emissions], "../ALFAM2/emission_3.xlsx")
