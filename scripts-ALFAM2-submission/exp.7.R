...
# Filter data
data <- data[data$date.time >= as.POSIXct("2022-09-28 11:06:36", tz="UTC"), ]
...
data$treat <- mapvalues(data$treat, from = c('1', '6', '9'), to = c(rep('AD-FB', 3)))
data$treat <- mapvalues(data$treat, from = c('3', '8', '10'), to = c(rep('AD-F', 3)))
data$treat <- mapvalues(data$treat, from = c('4', '5', '12'), to = c(rep('soil', 3)))
data$treat <- mapvalues(data$treat, from = c('2'), to = c('bg 1')) 
...
data$replicate <- mapvalues(data$replicate, from = c('1', '3', '4' ), to = rep('1', 3))
...
#tunnels data 
data <- data[data$treat == 'AD-FB' | data$treat == 'AD-F'| data$treat == 'soil', ]
...
#write_xlsx(data[, cols_to_keep_emissions], "../ALFAM2/emission_7.xlsx")
