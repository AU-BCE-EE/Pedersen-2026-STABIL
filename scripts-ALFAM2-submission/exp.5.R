...
data <- rbind(data)
#re-order by date, otherwise 2022-05-01 will be first and before month 04 data
...
# Filter data and match Picarro and real time
data <- data[data$date.time >= as.POSIXct("2022-04-26 12:21:36", tz="UTC"), ]
...
data$treat <- mapvalues(data$treat, from = c('2', '6', '12'), to = c(rep('PS', 3)))
data$treat <- mapvalues(data$treat, from = c('5', '8', '10'), to = c(rep('PS-L', 3)))
data$treat <- mapvalues(data$treat, from = c('1', '4', '9'), to = c(rep('PS-F', 3)))
...
data$replicate <- mapvalues(data$replicate, from = c('1', '2', '5' ), to = rep('1', 3))
data$replicate <- mapvalues(data$replicate, from = c('6', '4', '8' ), to = rep('2', 3))
...
#write_xlsx(data[, cols_to_keep_emissions], "../ALFAM2/emission_5.xlsx")
