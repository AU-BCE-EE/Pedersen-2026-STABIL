...
# Filter data
data <- data[data$date.time >= as.POSIXct("2022-04-12 11:24:13", tz="UTC"), ]
...
data$treat <- mapvalues(data$treat, from = c('2', '5', '12'), to = c(rep('PS', 3)))
data$treat <- mapvalues(data$treat, from = c('1', '8', '9'), to = c(rep('PS-L', 3)))
data$treat <- mapvalues(data$treat, from = c('4', '6', '10'), to = c(rep('PS-F', 3)))
...
data$replicate <- mapvalues(data$replicate, from = c('6', '5', '8' ), to = rep('2', 3))
...
#cols_to_keep_emission <- c("project", "exp", "field", "plot.number", "treatment", 
...
#write_xlsx(data[, cols_to_keep_emission], "../ALFAM2/emission_4.xlsx")
