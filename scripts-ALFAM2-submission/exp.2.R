...
#Filter for start time and synchroniza Picarro time and real time
data <- data[data$date.time >= as.POSIXct("2022-04-19 11:36:52", tz="UTC"), ]
data$date.time <- data$date.time + hours(1) + minutes(52)
...
data$treat <- mapvalues(data$treat, from = c('5', '8', '10'), to = c(rep('AD', 3)))
data$treat <- mapvalues(data$treat, from = c('2', '6', '9'), to = c(rep('AD-F', 3)))
data$treat <- mapvalues(data$treat, from = c('1', '4', '12'), to = c(rep('AD-L', 3)))
...
data$replicate <- mapvalues(data$replicate, from = c('5', '2', '1' ), to = rep('1', 3))
data$replicate <- mapvalues(data$replicate, from = c('4', '6', '8' ), to = rep('2', 3))
data$replicate <- mapvalues(data$replicate, from = c('9', '10', '12' ), to = rep('3', 3))
...
data$far.loc <- ''
data$id
ggplot(data, aes(elapsed.time, NH3.value, color = plot.number)) + geom_point()
...
#write_xlsx(data[, cols_to_keep_emissions], "../ALFAM2/emission_2.xlsx")
