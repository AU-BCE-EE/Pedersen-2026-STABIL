...
#Filter for experiment start and synchornize Picarro and real time
data <- data[data$date.time >= as.POSIXct("2022-10-19 11:27:36", tz="UTC") , ]
...
data <- data[data$id == '3' | data$id == '4' | data$id == '5' | data$id == '6' | data$id == '9' | data$id == '10' | data$id == '11' | 
               data$id == '12' | data$id == '15'| data$id == '16'| data$id == '17'| data$id == '18' ,]
...
data$treat <- mapvalues(data$treat, from = c('4', '9', '18'), to = c(rep('PS', 3)))
data$treat <- mapvalues(data$treat, from = c('3', '10', '15'), to = c(rep('PS-L', 3)))
data$treat <- mapvalues(data$treat, from = c('6', '12', '16'), to = c(rep('PS-F', 3)))
data$treat <- mapvalues(data$treat, from = c('5'), to = c('bg 1')) 
data$treat <- mapvalues(data$treat, from = c('11'), to = c('bg 2'))
data$treat <- mapvalues(data$treat, from = c('17'), to = c('bg 3'))


data$replicate <- data$id
data$replicate <- mapvalues(data$replicate, from = c('4', '3', '6' ), to = rep('1', 3))
data$replicate <- mapvalues(data$replicate, from = c('9', '10', '12' ), to = rep('2', 3))
data$replicate <- mapvalues(data$replicate, from = c('18', '15', '16' ), to = rep('3', 3))


# background dataset check for outliers
data.bg <- data[data$treat == 'bg 1' | data$treat == 'bg 2'| data$treat == 'bg 3', ]
View(data.bg)
data.bg<-filter(data.bg, elapsed.time<=138)
 
#delete at time 0 bg 2 as it is much higher than the others. 
data.bg <- data.bg[!(data.bg$treat == 'bg 2' & data.bg$elapsed.time == 0), ]
#tunnels data 
data <- data[data$treat == 'PS' | data$treat == 'PS-L'| data$treat == 'PS-F', ]
...
data<-filter(data, elapsed.time<=138)
...
#write_xlsx(data[, cols_to_keep_emissions], "../ALFAM2/emission_6.xlsx")
